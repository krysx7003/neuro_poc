...
def process_single_decision(folder_path: Path, output_file: Path) -> dict:
    ...
    if not all(f.exists() for f in required_files):
        raise FileNotFoundError(
            f"Missing files in {folder_path}: {[f.name for f in required_files if not f.exists()]}"
        )
    ...
    doc = {
        ...
        "year": 0,
        "entities": [],
        "content_text": parse_xml_content(xml_path),
        "meta": index_data,
        "toc": toc_data,
    ...
    doc.update(
        {
            "title": parsed_meta["name"],
            "title_full": parsed_meta["title_full"],
            "keywords": parsed_meta["keywords"],
            "keywords_list": parsed_meta["keywords_list"],
            "entities": parsed_meta["entities"],
    ...
    if parsed_meta["refs"]:
        ...
        def _find_relation(refs_full, sig):
            return next(
                (r.get("relation", "refers") for r in refs_full if r.get("signature") == sig),
                "refers",
            )
        ...
        doc["related_legislation"] = [
        ...
        ] + [
            {
                "type": "eu_act",
                "signature": s,
                "relation": _find_relation(refs_parsed["refs_full"], s),
            }
...
def batch_process(root_dir: Path, output_file: Path):
    ...
    for subfolder in root_dir.iterdir():
        ...
        try:
            ...
            sig = doc["signature"]
            print(
                f"✅ {sig:25} ({len(doc['content_text']):6,} chars, {len(doc['refs_full'])} refs)"
            )
    ...
    print("   Ready for uodo_scraper.py pipeline!")
