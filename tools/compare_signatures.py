...
def load_signatures(jsonl_file: Path) -> set[str]:
    ...
    try:
        ...
        print(
            f"✅ {jsonl_file.name}: {len(signatures):,} unique signatures from {line_count:,} lines",
            flush=True,
        )
        return signatures
...
def analyze_matches(file1: Path, file2: Path):
    ...
    print(
        f"✅ MATCHES: {len(matches):,} ({match_pct1:5.1f}% of {file1.name}, {match_pct2:5.1f}% of {file2.name})",
        flush=True,
    )
    if matches:
        print("   First 10 matches:", flush=True)
    ...
    print(
        f"\n➡️  UNIQUE in {file1.name}: {len(only1):,} ({len(only1) / total1 * 100:.1f}%)",
        flush=True,
    )
    if only1:
    ...
    print(
        f"➡️  UNIQUE in {file2.name}: {len(only2):,} ({len(only2) / total2 * 100:.1f}%)", flush=True
    )
    if only2:
